...
fn take_snapshot(control: &SnapshotControl) -> Value {
    ...
    // Marking the stripes is just atomics, so the freeze does it here rather
    // than asking the worker: the worker can be inside a slow push to a fork
    // that has gone away, and a snapshot should not wait on that.
